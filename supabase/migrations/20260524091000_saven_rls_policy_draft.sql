...
alter table public.saven_incident_actions enable row level security;
...
create policy "saven incident actions admin only"
on public.saven_incident_actions
for all
using (public.is_saven_admin())
with check (public.is_saven_admin());
...
using (
  ...
  or exists (
    select 1 from public.saven_profiles p
    where p.id = saven_events.profile_id
      and p.owner_user_id = auth.uid()
  )
);
