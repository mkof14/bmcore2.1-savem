...
function SavenCommandsPage({ openPage }: { openPage: (pageId: SavenPageId) => void }) {
  const commandCatalog: Array<{
    ...
    { id: 'caregiver', group: 'People', label: 'Caregiver task', target: 'Maya Carter', command: 'Hey SAVEN, assign the next support task to Maya and require verification.', page: 'app-circle', color: 'blue', icon: UsersRound, proof: 'Caregiver receives task and verification rule.', route: ['Need', 'Assign', 'Confirm'] },
    { id: 'nurse', group: 'Clinical', label: 'Nurse follow-up', target: 'Nurse Olivia Grant', command: 'Hey SAVEN, request nurse follow-up and send the recovery context.', page: 'app-care-routes', color: 'green', icon: PhoneCall, proof: 'Nurse receives recovery context and next action.', route: ['Context', 'Nurse', 'Update'] },
    { id: 'doctor', group: 'Clinical', label: 'Doctor summary', target: 'Dr. Elena Morris', command: 'Hey SAVEN, prepare a clinical summary for doctor review.', page: 'app-care-routes', color: 'blue', icon: ShieldCheck, proof: 'Summary is prepared before outreach.', route: ['Collect', 'Summarize', 'Review'] },
    { id: 'family', group: 'People', label: 'Family update', target: 'Daniel Roberts', command: 'Hey SAVEN, send the family a calm daily update and do not repeat non-urgent alerts.', page: 'app-care-routes', color: 'gold', icon: UsersRound, proof: 'Family gets one clear digest.', route: ['Digest', 'Family', 'Quiet'] },
    { id: 'robot', group: 'Physical support', label: 'Robot readiness', target: 'Mobility robot', command: 'Hey SAVEN, check robot readiness and keep physical action approval locked.', page: 'app-robots', color: 'gold', icon: Bot, proof: 'Robot cannot act until approval and verification are ready.', route: ['Readiness', 'Lock', 'Approve'] },
    { id: 'device', group: 'Physical support', label: 'Device telemetry', target: 'Wearable + home sensors', command: 'Hey SAVEN, check device telemetry and use it only for verification.', page: 'app-devices', color: 'green', icon: Cpu, proof: 'Device data supports confirmation, not uncontrolled action.', route: ['Signal', 'Verify', 'Record'] },
    { id: 'environment', group: 'Rules', label: 'Room rules', target: 'Home Recovery', command: 'Hey SAVEN, show environment permissions before any support action.', page: 'app-environments', color: 'blue', icon: Waypoints, proof: 'Room, privacy, and access rules are checked first.', route: ['Room', 'Rule', 'Permission'] },
    { id: 'emergency', group: 'Escalation', label: 'Emergency path', target: 'Emergency services', command: 'Hey SAVEN, show emergency escalation rules and who is contacted first.', page: 'app-care-routes', color: 'red', icon: AlertCircle, proof: 'Escalation order is visible before action.', route: ['Urgent', 'Escalate', 'Log'] },
  ];
  ...
  const colorMap: Record<string, string> = {
    blue: 'border-blue-200 bg-blue-50/74 text-blue-950 dark:border-blue-300/20 dark:bg-blue-500/10 dark:text-blue-100',
    green: 'border-emerald-200 bg-emerald-50/74 text-emerald-950 dark:border-emerald-300/20 dark:bg-emerald-500/10 dark:text-emerald-100',
    gold: 'border-amber-200 bg-amber-50/74 text-amber-950 dark:border-amber-300/20 dark:bg-amber-500/10 dark:text-amber-100',
    red: 'border-red-200 bg-red-50/74 text-red-950 dark:border-red-300/20 dark:bg-red-500/10 dark:text-red-100',
  };

  return (
    <div className="space-y-6">
      <section className="overflow-hidden rounded-[2rem] border border-blue-200/70 bg-[radial-gradient(circle_at_10%_12%,rgba(59,130,246,0.22),transparent_30%),radial-gradient(circle_at_90%_18%,rgba(249,115,22,0.16),transparent_28%),linear-gradient(135deg,rgba(255,255,255,0.96),rgba(239,246,255,0.82))] p-6 shadow-lg shadow-blue-950/5 dark:border-blue-300/20 dark:bg-[radial-gradient(circle_at_10%_12%,rgba(59,130,246,0.28),transparent_30%),radial-gradient(circle_at_90%_18%,rgba(249,115,22,0.18),transparent_28%),linear-gradient(135deg,rgba(6,16,31,0.98),rgba(15,23,42,0.9))] dark:ring-1 dark:ring-blue-300/15">
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_430px] xl:items-stretch">
          <div>
            ...
            <h2 className="mt-3 text-4xl font-semibold tracking-tight text-slate-950 dark:text-white md:text-5xl">Voice and text command center.</h2>
            <p className="mt-4 max-w-3xl text-base leading-7 text-slate-600 dark:text-slate-300">Give practical commands to people, nurses, doctors, family, robots, devices, environments, and emergency routes. Every command shows where it goes and what proof SAVEN expects.</p>
            <div className="mt-5 grid gap-3 sm:grid-cols-3">
              <div className="rounded-3xl bg-white/74 p-4 shadow-inner ring-1 ring-white/70 dark:bg-slate-950/54 dark:ring-white/10">
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">Input</p>
                <p className="mt-2 text-lg font-semibold text-slate-950 dark:text-white">Voice or text</p>
              </div>
              <div className="rounded-3xl bg-white/74 p-4 shadow-inner ring-1 ring-white/70 dark:bg-slate-950/54 dark:ring-white/10">
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">Routing</p>
                <p className="mt-2 text-lg font-semibold text-slate-950 dark:text-white">Person + service</p>
              </div>
              <div className="rounded-3xl bg-white/74 p-4 shadow-inner ring-1 ring-white/70 dark:bg-slate-950/54 dark:ring-white/10">
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">Safety</p>
                <p className="mt-2 text-lg font-semibold text-slate-950 dark:text-white">Verify first</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-[1.75rem] border border-white/70 bg-white/82 p-4 shadow-sm dark:border-white/10 dark:bg-slate-950/68">
            <div className="flex items-start gap-3">
              <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-slate-950 text-white shadow-sm dark:bg-white dark:text-slate-950">
                <MessageSquareText className="h-5 w-5" />
              </span>
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">Text command composer</p>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
          ...
          {commandCatalog.map((item) => {
            ...
            return (
              <button
                ...
                className={(activeCard ? 'scale-[1.01] border-blue-300 shadow-xl shadow-blue-950/10 ring-4 ring-blue-500/10 dark:border-blue-300/40' : 'shadow-sm hover:-translate-y-0.5 hover:shadow-lg') + ' rounded-[1.5rem] border p-4 text-left transition-all ' + colorMap[item.color]}
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.15em] opacity-70">{item.group}</p>
                    <h3 className="mt-2 text-lg font-semibold">{item.label}</h3>
                    <p className="mt-1 text-sm font-semibold opacity-75">{item.target}</p>
                  </div>
                ...
                <p className="mt-4 line-clamp-3 text-sm leading-6 opacity-86">{item.command}</p>
              </button>
            );
          })}
        </div>
        ...
        <aside className="rounded-[2rem] border border-slate-200 bg-white/82 p-5 shadow-sm dark:border-white/10 dark:bg-slate-950/70">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Command route</p>
          <h3 className="mt-2 text-2xl font-semibold text-slate-950 dark:text-white">{active.target}</h3>
          <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{active.proof}</p>
          <div className="mt-5 grid gap-3">
            {active.route.map((step, index) => (
              <div key={step} className="flex items-center gap-3">
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-blue-600 text-sm font-semibold text-white shadow-sm">{index + 1}</span>
          ...
          <div className="mt-5 rounded-3xl bg-slate-950 p-4 text-white shadow-inner dark:bg-white dark:text-slate-950">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] opacity-65">Current command</p>
            <p className="mt-2 text-sm font-semibold leading-6">{draftCommand}</p>
          </div>
        </aside>
      </section>
    </div>
...
: { openPage: (pageId: SavenPageId) => void }) {
  const commandGroups = [
    {
      title: 'People',
      text: 'Route work to caregivers, family, nurse, doctor, and emergency paths.',
      tone: 'blue',
      items: [
        { label: 'Caregiver', target: 'Maya Carter', command: 'Hey SAVEN, assign the next support task to Maya and require verification.', page: 'app-circle' as SavenPageId },
        { label: 'Nurse', target: 'Nurse Olivia Grant', command: 'Hey SAVEN, request nurse follow-up and send the recovery context.', page: 'app-care-routes' as SavenPageId },
        { label: 'Doctor', target: 'Dr. Elena Morris', command: 'Hey SAVEN, prepare a clinical summary for doctor review.', page: 'app-care-routes' as SavenPageId },
        { label: 'Emergency', target: 'Emergency path', command: 'Hey SAVEN, show emergency escalation rules and who is contacted first.', page: 'app-care-routes' as SavenPageId },
      ],
    },
    {
      title: 'Robots and devices',
      text: 'Check readiness, telemetry, safety locks, and physical support permissions.',
      tone: 'gold',
      items: [
        { label: 'Robot readiness', target: 'Mobility robot', command: 'Hey SAVEN, check robot readiness and keep physical action approval locked.', page: 'app-robots' as SavenPageId },
        { label: 'Device telemetry', target: 'Wearable + home sensors', command: 'Hey SAVEN, check device telemetry and use it only for verification.', page: 'app-devices' as SavenPageId },
        { label: 'Room rules', target: 'Home environment', command: 'Hey SAVEN, show environment permissions before any support action.', page: 'app-environments' as SavenPageId },
      ],
    },
    {
      title: 'Daily support',
      text: 'Start daily support, verify completed actions, and update continuity.',
      tone: 'green',
      items: [
        { label: 'Today status', target: 'Daily support', command: 'Hey SAVEN, show today support status and next action.', page: 'app-today' as SavenPageId },
        { label: 'Confirm action', target: 'Verification', command: 'Hey SAVEN, confirm walking session completed and wait for verification.', page: 'app-verification' as SavenPageId },
        { label: 'Continuity', target: 'Continuity operations', command: 'Hey SAVEN, update continuity and show the next support window.', page: 'app-continuity' as SavenPageId },
      ],
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <section className="overflow-hidden rounded-[2rem] border border-blue-200/70 bg-[radial-gradient(circle_at_10%_12%,rgba(59,130,246,0.22),transparent_30%),radial-gradient(circle_at_90%_18%,rgba(249,115,22,0.16),transparent_28%),linear-gradient(135deg,rgba(255,255,255,0.96),rgba(239,246,255,0.82))] p-7 shadow-lg shadow-blue-950/5 dark:border-blue-300/20 dark:bg-[radial-gradient(circle_at_10%_12%,rgba(59,130,246,0.28),transparent_30%),radial-gradient(circle_at_90%_18%,rgba(249,115,22,0.18),transparent_28%),linear-gradient(135deg,rgba(6,16,31,0.98),rgba(15,23,42,0.9))] dark:ring-1 dark:ring-blue-300/15">
        <div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.22em] text-blue-700 dark:text-blue-200">SAVEN voice and text commands</p>
            <h2 className="mt-3 text-4xl font-semibold tracking-tight text-slate-950 dark:text-white md:text-5xl">Command SAVEN directly.</h2>
            <p className="mt-4 max-w-3xl text-base leading-7 text-slate-600 dark:text-slate-300">Use this page to speak or type commands for workers, nurse, doctor, family, devices, robots, environments, verification, and emergency escalation.</p>
          </div>
          <button onClick={() => openPage('app-settings')} className="rounded-full bg-slate-950 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-slate-950/15 transition-all hover:-translate-y-0.5 hover:bg-blue-700 dark:bg-white dark:text-slate-950 dark:hover:bg-blue-100">
            Open real mic test
          </button>
        </div>
      </section>
      ...
      <div className="grid gap-5 xl:grid-cols-3">
        {commandGroups.map((group) => {
          const tone =
            group.tone === 'green'
              ? 'border-emerald-200 bg-emerald-50/72 dark:border-emerald-300/20 dark:bg-emerald-500/10'
              : group.tone === 'gold'
                ? 'border-amber-200 bg-amber-50/72 dark:border-amber-300/20 dark:bg-amber-500/10'
                : 'border-blue-200 bg-blue-50/72 dark:border-blue-300/20 dark:bg-blue-500/10';
          return (
            <section key={group.title} className={'rounded-[2rem] border p-5 shadow-sm ' + tone}>
              <h3 className="text-2xl font-semibold text-slate-950 dark:text-white">{group.title}</h3>
              <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{group.text}</p>
              <div className="mt-5 grid gap-3">
                {group.items.map((item) => (
                  <article key={item.label} className="rounded-3xl border border-white/80 bg-white/86 p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-lg dark:border-white/10 dark:bg-slate-950/72">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{item.label}</p>
                        <p className="mt-1 break-words text-sm font-semibold text-slate-950 dark:text-white">{item.target}</p>
                      </div>
                      <Mic className="h-5 w-5 shrink-0 text-blue-600 dark:text-blue-200" />
                    </div>
                    <p className="mt-3 rounded-2xl bg-slate-950 px-3 py-3 text-sm font-semibold leading-6 text-white shadow-inner dark:bg-white dark:text-slate-950">{item.command}</p>
                    <button onClick={() => openPage(item.page)} className="mt-3 rounded-full bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-all hover:-translate-y-0.5 hover:bg-blue-500">
