...
const savenMockState: SavenMockState = {
...
const savenControlApiMock = {
  createTask(taskTitle: string): SavenControlApiResult {
    return {
      ...
      payload: {
        taskTitle,
        personId: savenMockState.activePersonId,
        lifecycle: 'created',
  ...
  assignTask(taskId: string, ownerId: string): SavenControlApiResult {
    const owner = savenMockState.people.find((person) => person.id === ownerId);
    return {
  ...
  verifyAction(taskId: string, verifierId: string): SavenControlApiResult {
    const verifier = savenMockState.people.find((person) => person.id === verifierId);
    return {
  ...
  escalate(level: SavenMockEscalation['level']): SavenControlApiResult {
    const escalation = savenMockState.escalations.find((item) => item.level === level);
    return {
      operation: 'escalate',
  ...
  updateContinuity(taskId: string): SavenControlApiResult {
    return {
      ...
      payload: {
        taskId,
        currentScore: savenMockState.continuity.score,
        openVerifications: savenMockState.continuity.openVerifications,
      },
    };
  },
};
...
function SavenDeveloperReadinessStack() {
  const panels = [
